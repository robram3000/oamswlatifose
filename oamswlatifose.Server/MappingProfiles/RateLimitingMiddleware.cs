            ...
            try
            {
                ...
                if (!tracker.IsRequestAllowed(_maxRequests, _timeWindow))
                {
                    ...
                    context.Response.Headers["X-RateLimit-Reset"] =
                        tracker.GetResetTime().ToUnixTimeSeconds().ToString();
                    context.Response.Headers["Retry-After"] =
                        tracker.GetTimeUntilReset().Seconds.ToString();

                    await context.Response.WriteAsJsonAsync(new
                    {
                        Success = false,
                        Message = "Too many requests. Please try again later.",
                        RetryAfterSeconds = tracker.GetTimeUntilReset().Seconds
                    });
        ...
        private class ClientRequestTracker
        {
            ...
            public DateTimeOffset GetResetTime()
            {
                lock (_lock)
                {
                    ...
                    return new DateTimeOffset(oldestRequest.Add(_timeWindow));
                }
            }
